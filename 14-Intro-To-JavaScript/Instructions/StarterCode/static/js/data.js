...
let dataHTML = data.map(data => `<tr><th>${data.datetime}</th><th>${data.city}</th><th>${data.state}</th><th>${data.country}</th><th>${data.shape}</th><th>${data.durationMinutes}</th><th>${data.comments}</th></tr>`)
document.querySelector('table tbody').innerHTML = dataHTML.join("");